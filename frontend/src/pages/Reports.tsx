...
export const Reports: React.FC = () => {
  ...
  const attendanceColumns: Column<any>[] = [
    ...
    { header: 'Employee', accessor: (r) => `${r.employee_name} (${r.employee_code})` },
    { header: 'Project / Task', accessor: (r) => r.task_name ? `${r.task_name} (${r.project_name || ''})` : 'General Duty' },
    { header: 'Check In', accessor: (r) => new Date(r.check_in_time).toLocaleTimeString() },
    { header: 'Check Out', accessor: (r) => r.check_out_time ? new Date(r.check_out_time).toLocaleTimeString() : '-' },
    { header: 'GPS Address', accessor: (r) => r.in_address || 'GPS Logged' },
    { header: 'Working Hours', accessor: (r) => `${Number(r.total_working_hours).toFixed(2)} hrs` },
    {
      header: 'Status',
      accessor: (r) => (
  ...
  const projectColumns: Column<any>[] = [
    ...
    { header: 'Pending', accessor: 'pending_tasks' },
    { header: 'In Progress', accessor: 'in_progress_tasks' },
    { header: 'Completed', accessor: 'completed_tasks' },
    { header: 'Delayed', accessor: 'delayed_tasks' },
    { header: 'Est. Hours', accessor: (r) => `${Number(r.total_estimated_hours).toFixed(2)}h` },
    { header: 'Actual Hours', accessor: (r) => `${Number(r.total_actual_hours).toFixed(2)}h` },
    {
      header: 'Progress %',
  ...
  const taskColumns: Column<any>[] = [
    ...
    { header: 'Workers (Assigned / Req)', accessor: (r) => `${r.assigned_worker_count} / ${r.required_worker_count}` },
    { header: 'Estimated Hours', accessor: (r) => `${Number(r.estimated_hours).toFixed(2)}h` },
    { header: 'Actual Logged Hours', accessor: (r) => `${Number(r.actual_hours).toFixed(2)}h` },
  ...
  const paymentColumns: Column<any>[] = [
    { header: 'Identifier / Name', accessor: (r) => r.employee_name || r.project_name || r.task_name || r.attendance_date },
    { header: 'Hours Logged', accessor: (r) => `${Number(r.total_hours || r.actual_hours || 0).toFixed(2)} hrs` },
    {
    ...
    <div>
      <div className="page-header">
        <div>
          ...
          <p className="page-subtitle">Generate, filter, and export detailed analytical reports for operational management</p>
        </div>
      </div>
      ...
      {isLoading ? (
        ...
        <div className="glass-card">
          <DataTable
            ...
            exportFilename={`${reportCategory}_report_export.csv`}
          />
        </div>
      )}
    </div>
